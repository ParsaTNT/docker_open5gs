...
#nef
openssl genrsa -out $1/nef.key.pem 1024
openssl req -new -batch -out nef.csr.pem -key $1/nef.key.pem -subj /CN=nef.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=Open5GS/OU=Tests
openssl ca -cert $1/cacert.pem -days 3650 -keyfile cakey.pem -in nef.csr.pem -out $1/nef.cert.pem -outdir . -batch
